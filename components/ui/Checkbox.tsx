...
export function Checkbox({ checked, onToggle, label, disabled }: Props) {
  ...
  return (
    <TouchableOpacity
      ...
      activeOpacity={0.7}
    >
      <View style={[styles.box, !hasLabel && styles.boxWithoutLabel, checked && styles.checked]}>
        {checked && <Text style={styles.tick}>✓</Text>}
      </View>
      {hasLabel && (
        <Text style={[styles.label, checked && styles.doneLabel, disabled && { opacity: 0.4 }]}>
          {label}
        </Text>
      )}
    </TouchableOpacity>
